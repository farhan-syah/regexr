...
///
/// A unit is one choice point — the quantity that actually explodes — in both
/// the interpreter and the generated code; the forward scanning between two
/// choice points is already bounded by the input and the program. Exhausting the
/// default costs roughly 0.4s on the generated code and 2.5s on the
/// interpreter, so a caller that needs a tighter ceiling should set its own.
